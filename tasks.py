...
def coverage(c):
    ...
    """
    c.run("coverage run --source assert_element runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/assert_element.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ assert_element")
...
def lint(c):
    ...
    """
    c.run("flake8 assert_element tests")
...
def release(c, bumpsize=""):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=assert_element.__version__))
    c.run("git push --tags")
